...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var pizzaContainers = document.getElementsByClassName("randomPizzaContainer"); //classname selector has better speed than querySelectorAll
    var windowWidth = document.querySelector("#randomPizzas").offsetWidth;
    var newSize = sizeSwitcher(size);

    //only calculate the newwidth for one pizza since all the pizzas are the same width
    var oldWidth = pizzaContainers[0].offsetWidth;
    var oldSize = oldWidth / windowWidth;
    var dx = (newSize - oldSize) * windowWidth;
    var newwidth = (oldWidth + dx) + 'px';

    //set pizzas to the new width
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var s = 256;
  var rows = height/s;
  var pizzas = rows * cols; //only generate enough pizzas to fill the screen at 100% zoom
  var elem;
